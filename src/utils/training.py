...
def get_manual_select_model(args, X, y):
...
def train(model, device, args):
    ...
    pbar = tqdm(range(args.training.epochs))

    for t in pbar:
        data_dict = get_data(alphas=args.data.data_alphas, 
                             N=args.data.N, 
                             d_d=args.data.d_d, 
                             train_samp_per_class=args.data.train_samp_per_class)
        ...
        if t % args.training.save_every_epochs == 0 and t > 0:
            torch.save(model.state_dict(), f"{args.out_dir}/model_step={t}.pt")
